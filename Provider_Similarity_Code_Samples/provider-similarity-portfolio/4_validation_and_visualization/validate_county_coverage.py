...
PRIMARY_SPECIALTY_FILTER = 'SPECIALTY_A'
...
county_df = pd.read_parquet('data15.parquet')
print(f"County data: {county_df.shape}")

county_df['county_state'] = county_df['county_nm'].fillna('') + '|' + county_df['state_code'].fillna('')
...
county_summary.to_csv(f'data16_{PRIMARY_SPECIALTY_FILTER}_{SIMILARITY_THRESHOLD}.csv', 
                      index=False)
print(f"✓ Saved: county_level_alternatives_{PRIMARY_SPECIALTY_FILTER}_threshold_{SIMILARITY_THRESHOLD}.csv")

state_summary.to_csv(f'data69_{PRIMARY_SPECIALTY_FILTER}_{SIMILARITY_THRESHOLD}.csv', 
                     index=False)
print(f"✓ Saved: state_level_alternatives_{PRIMARY_SPECIALTY_FILTER}_threshold_{SIMILARITY_THRESHOLD}.csv")

provider_alt_df.to_csv(f'data57_{PRIMARY_SPECIALTY_FILTER}_{SIMILARITY_THRESHOLD}.csv', 
                       index=False)
