...
ATTENTION_CACHE_FILE = 'data9.pkl'
print(f"\nDevice: {DEVICE}")
...
pairs_df = pd.read_csv('data6.csv')
print(f"  Loaded {len(pairs_df):,} pairs")

print("\nLoading embeddings...")
embeddings_df = pd.read_parquet('data22.parquet')
...
class ProviderGAT(nn.Module):
    def __init__(self, code_dim, hidden_dim, num_specialties, num_heads=4, dropout=0.3):
        ...
        self.W_heads = nn.ModuleList([
            nn.Linear(code_dim, hidden_dim, bias=False) for _ in range(num_heads)
        ])
...
for idx in tqdm(range(len(pairs_df)), desc="Computing"):
    ...
    p_codes = p_data['codes']
    a_codes = a_data['codes']
    p_attention = np.array(p_data['attention']) if p_data['attention'] else np.array([])
    a_attention = np.array(a_data['attention']) if a_data['attention'] else np.array([])
    p_claims = p_data['claims']
    a_claims = a_data['claims']
    
    # Primary's top 5 codes
    if len(p_codes) > 0 and len(p_attention) > 0:
        if len(p_attention) <= 5:
        ...
        p_top5_att = [float(p_attention[i]) for i in p_top_indices if i < len(p_attention)]
    ...
    if len(a_codes) > 0 and len(a_attention) > 0:
        if len(a_attention) <= 5:
        ...
        a_top5_att = [float(a_attention[i]) for i in a_top_indices if i < len(a_attention)]
    ...
    if len(p_codes) > 0 and len(a_codes) > 0 and len(p_attention) > 0 and len(a_attention) > 0:
        if len(p_attention) <= 10:
            p_top10_indices = np.argsort(p_attention)[::-1]
        else:
            p_top10_indices = np.argpartition(p_attention, -10)[-10:]
        
        if len(a_attention) <= 10:
