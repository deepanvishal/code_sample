...
code_totals = procedure_df.groupby('code')['claims'].sum()
procedure_df['code_total'] = procedure_df['code'].map(code_totals)
procedure_df['share_squared'] = (procedure_df['claims'] / procedure_df['code_total']) ** 2
herfindahl = procedure_df.groupby('code')['share_squared'].sum().reset_index()
herfindahl.columns = ['code', 'herfindahl']
code_stats = code_stats.merge(herfindahl, on='code', how='left')
procedure_df.drop(['code_total', 'share_squared'], axis=1, inplace=True)
