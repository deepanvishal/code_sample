...
# =============================================================================
# LOSS OPTION - SELECT ONE
# =============================================================================
LOSS_OPTION = 2  # Change to 1, 2, 3, or 4

# Option 1: 0.5 * jaccard + 0.5 * coverage
# Option 2: 0.5 * jaccard + 0.5 * weighted_jaccard
# Option 3: weighted_jaccard only
# Option 4: 0.33 * jaccard + 0.33 * weighted_jaccard + 0.33 * coverage

LOSS_DESCRIPTIONS = {
    1: "0.5 * jaccard + 0.5 * coverage",
    2: "0.5 * jaccard + 0.5 * weighted_jaccard",
    3: "weighted_jaccard only",
    4: "0.33 * jaccard + 0.33 * weighted_jaccard + 0.33 * coverage"
}

print(f"Model: {MODEL_NAME}")
print(f"Loss Option: {LOSS_OPTION} = {LOSS_DESCRIPTIONS[LOSS_OPTION]}")
...
# Verify all metrics are 0-1 scale
print(f"Code overlap (Jaccard): min={pair_labels_df['code_overlap'].min():.4f}, max={pair_labels_df['code_overlap'].max():.4f}")
print(f"Weighted Jaccard:       min={pair_labels_df['weighted_jaccard'].min():.4f}, max={pair_labels_df['weighted_jaccard'].max():.4f}")
...
assert pair_labels_df['weighted_jaccard'].max() <= 1.0, "Weighted Jaccard not normalized!"
assert pair_labels_df['coverage_overlap'].max() <= 1.0, "Coverage overlap not normalized!"
print("✓ All metrics in 0-1 range")

# Compute target based on LOSS_OPTION
if LOSS_OPTION == 1:
    pair_labels_df['combined_target'] = 0.5 * pair_labels_df['code_overlap'] + 0.5 * pair_labels_df['coverage_overlap']
elif LOSS_OPTION == 2:
    pair_labels_df['combined_target'] = 0.5 * pair_labels_df['code_overlap'] + 0.5 * pair_labels_df['weighted_jaccard']
elif LOSS_OPTION == 3:
    pair_labels_df['combined_target'] = pair_labels_df['weighted_jaccard']
elif LOSS_OPTION == 4:
    pair_labels_df['combined_target'] = (pair_labels_df['code_overlap'] + pair_labels_df['weighted_jaccard'] + pair_labels_df['coverage_overlap']) / 3
else:
    raise ValueError(f"Invalid LOSS_OPTION: {LOSS_OPTION}")

print(f"Target formula: {LOSS_DESCRIPTIONS[LOSS_OPTION]}")
...
# Keep dict for pin lookup (PINs are too large for array indexing)
# pin_to_idx already loaded from pickle
...
def similarity_loss_mse(embeddings, provider_ids, pin_to_idx, pair_to_target, pair_keys_set):
    """
    MSE loss between cosine similarity and combined overlap target.
    Optimized: minimal CPU work, sampling for large batches.
    """
    pids_np = provider_ids.numpy()
    
    # Dict lookup for valid providers
    valid_batch_idx = []
    valid_matrix_idx = []
    for i, pid in enumerate(pids_np):
        if pid in pin_to_idx:
            valid_batch_idx.append(i)
            valid_matrix_idx.append(pin_to_idx[pid])
    
    if len(valid_batch_idx) < 2:
        return torch.tensor(0.0, device=embeddings.device), 0
    
    valid_batch_idx = np.array(valid_batch_idx)
    valid_matrix_idx = np.array(valid_matrix_idx)
...
print(f"✓ Similarity loss: MSE regression on cosine sim vs target")
print(f"✓ Target: {LOSS_DESCRIPTIONS[LOSS_OPTION]}")
...
for epoch in range(MAX_EPOCHS):
    ...
    for pids, code_embs, masks, labels in train_loader:
        ...
        loss_sim, n_pairs = similarity_loss_mse(embeddings, pids, pin_to_idx, pair_to_target, pair_keys_set)
...
results = {
    ...
    'similarity_boost': SIMILARITY_BOOST, 'loss_option': LOSS_OPTION,
    'loss_description': LOSS_DESCRIPTIONS[LOSS_OPTION], 'model_name': MODEL_NAME
}
