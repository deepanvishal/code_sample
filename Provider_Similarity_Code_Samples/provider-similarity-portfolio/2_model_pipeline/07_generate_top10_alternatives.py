...
procedure_df = pd.read_parquet('data48.parquet')
print(f"Procedure data: {procedure_df.shape}")

diagnosis_df = pd.read_parquet('data19.parquet')
print(f"Diagnosis data: {diagnosis_df.shape}")

demo_df = pd.read_parquet('data17.parquet')
print(f"Demographics: {demo_df.shape}")

place_df = pd.read_parquet('data45.parquet')
print(f"Place: {place_df.shape}")

cost_df = pd.read_parquet('data14.parquet')
print(f"Cost: {cost_df.shape}")

pin_df = pd.read_parquet('data42.parquet')
print(f"PIN summary: {pin_df.shape}")

pin_names_df = pd.read_parquet('data3.parquet')
...
prov_spl_df = pd.read_parquet('data50.parquet')
print(f"Provider specialty: {prov_spl_df.shape}")

pin_to_specialty = dict(zip(prov_spl_df['PIN'], prov_spl_df['specialty_category_code']))
...
for chunk_idx in tqdm(range(n_chunks), desc="Processing chunks"):
    ...
    if chunk_idx == 0:
        chunk_df.to_csv('data5.csv', index=False, mode='w')
    else:
        chunk_df.to_csv('data5.csv', index=False, mode='a', header=False)
...
count_matrix.to_csv('data66.csv')
print("✓ Saved: specialty_category_count_matrix_me2vec.csv")

pct_matrix.to_csv('data67.csv')
...
combined_matrix.to_csv('data65.csv')
