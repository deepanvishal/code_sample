...
proc_emb_df = pd.read_parquet('data37.parquet')
diag_emb_df = pd.read_parquet('data38.parquet')
...
demographics = np.load('demo_vectors.npy')
place = np.load('plc_vectors.npy')
cost = np.load('cost_ctg_vectors.npy')
pin_summary = np.load('PIN_smry_vectors.npy')

all_pins_linear = np.load('all_pins.npy', allow_pickle=True).tolist()

print(f"\nLoaded linear towers:")
print(f"  Demographics: {demographics.shape}")
print(f"  Place: {place.shape}")
print(f"  Cost: {cost.shape}")
print(f"  PIN summary: {pin_summary.shape}")
...
final_demographics = np.array([demographics[linear_pin_to_idx[pin]] for pin in final_pins])
final_place = np.array([place[linear_pin_to_idx[pin]] for pin in final_pins])
final_cost = np.array([cost[linear_pin_to_idx[pin]] for pin in final_pins])
final_pin_summary = np.array([pin_summary[linear_pin_to_idx[pin]] for pin in final_pins])

print(f"\nFinal aligned data:")
print(f"  Procedure embeddings: {final_proc_embeddings.shape}")
print(f"  Diagnosis embeddings: {final_diag_embeddings.shape}")
print(f"  Demographics: {final_demographics.shape}")
print(f"  Place: {final_place.shape}")
print(f"  Cost: {final_cost.shape}")
print(f"  PIN summary: {final_pin_summary.shape}")
...
demographics_norm = scaler_demo.fit_transform(final_demographics)
place_norm = scaler_place.fit_transform(final_place)
cost_norm = scaler_cost.fit_transform(final_cost)
pin_norm = scaler_pin.fit_transform(final_pin_summary)
