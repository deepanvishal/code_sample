...
"""
HYBRID PROTOTYPE MODEL: CNP + LEARNED QUERY-DEPENDENT WEIGHTING (GAT VERSION)
==============================================================================
...
Input: GAT embeddings from combine_gat_towers.py
"""
...
print("\n" + "="*80)
print("HYBRID PROTOTYPE MODEL TRAINING (GAT VERSION)")
print("="*80)
...
# Load tower dims from metadata
with open('data21.pkl', 'rb') as f:
    metadata = pickle.load(f)

tower_ranges = metadata['tower_ranges']

# Convert to simple format
tower_dims = {
    'procedures': tower_ranges['tower1_procedures'],
    'diagnoses': tower_ranges['tower2_diagnoses'],
    'demographics': tower_ranges['tower3_demographics'],
    'place': tower_ranges['tower4_place'],
    'cost': tower_ranges['tower5_cost'],
    'pin': tower_ranges['tower6_pin']
}

pin_to_idx = {pin: idx for idx, pin in enumerate(all_pins)}
labeled_indices = [pin_to_idx[pin] for pin in pin_to_label.keys() if pin in pin_to_idx]
labeled_pins = [pin for pin in pin_to_label.keys() if pin in pin_to_idx]

unique_labels = sorted(set(pin_to_label[pin] for pin in labeled_pins))
...
for i, idx in enumerate(prototype_indices[:10]):
    ...
    print(f"  {i+1:2d}. PIN {pin} In-degree={in_deg:8.4f} Specialty={specialty}")
...
torch.save({
    ...
    'history': history
}, 'gat_trained_prototype_model.pth')
print("✓ Saved: gat_trained_prototype_model.pth")

with open('data26.pkl', 'wb') as f:
    pickle.dump({
...
print("✓ Saved: gat_cnp_prototypes.pkl")

with open('data28.pkl', 'wb') as f:
    pickle.dump(provider_to_prototype, f)
print("✓ Saved: gat_provider_to_prototype.pkl")

np.save('gat_cnp_in_degrees.npy', in_degrees)
print("✓ Saved: gat_cnp_in_degrees.npy")

np.save('gat_weighted_embeddings.npy', all_weighted_embeddings)
print("✓ Saved: gat_weighted_embeddings.npy")

with open('data27.pkl', 'wb') as f:
    pickle.dump({
        ...
        'labeled_providers': len(labeled_pins),
...
print("✓ Saved: gat_hybrid_model_metadata.pkl")
...
if len(labeled_pins) > 0:
    query_pin = random.choice(labeled_pins)
    ...
    cnp_scores = cnp_matrix[query_idx].copy()
    cnp_scores[query_idx] = 0
    
    top_10_indices = np.argsort(cnp_scores)[-10:][::-1]
    
    print(f"\nTop 10 similar providers (CNP):")
    print(f"{'Rank':<6s} {'PIN':<15s} {'CNP Score':<12s} {'Specialty':<30s}")
    print("-" * 70)
    
    for rank, idx in enumerate(top_10_indices, 1):
        similar_pin = all_pins[idx]
        cnp_score = cnp_scores[idx]
        specialty = pin_to_label.get(similar_pin, 'UNLABELED')
        match = "✓" if specialty == true_label else ""
        print(f"{rank:<6d} {similar_pin:<15} {cnp_score:<12.6f} {specialty:<30s} {match}")
    
    n_same_specialty = sum(1 for idx in top_10_indices if pin_to_label.get(all_pins[idx], '') == true_label)
    print(f"\n✓ {n_same_specialty}/10 are {true_label}")

print("\n" + "="*80)
print("HYBRID PROTOTYPE MODEL TRAINING COMPLETE (GAT VERSION)")
print("="*80)
print(f"\nTraining time: {total_training_time:.2f} sec ({total_training_time/60:.2f} min)")
print(f"CNP computation time: {cnp_time:.2f} sec")
print(f"\nSaved files:")
print("  - gat_trained_prototype_model.pth")
print("  - gat_cnp_prototypes.pkl")
print("  - gat_provider_to_prototype.pkl")
print("  - gat_cnp_in_degrees.npy")
print("  - gat_weighted_embeddings.npy")
print("  - gat_hybrid_model_metadata.pkl")
print("\nReady for inference!")
